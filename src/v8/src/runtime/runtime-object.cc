  ...
  if (!result.is_null() && it.IsFound() && key->IsString()) {
    ...
    if (is_key_tainted) {
      ...
      if (handle_result->IsHeapObject()) {
        ...
        std::cout << "ObjTaintedDueToTaintKey! ObjectAddress: ";
        handle_result->Print();
        std::cout << " KeyTaintType " << tainttracking::TaintTypeToString(key_taint_type) << " KeyIs ";
    ...
    if (key->IsString() && value->IsString()) {
      if (object->IsHeapObject() && object->IsPrototypePolluted()) {
        ...
        if (is_key_tainted && is_value_tainted) {
          ...
          std::cout << "ppFOUND! ObjectAddress ";
          object->Print();// Failed when using object->address();
          std::cout << " KeyTaintType " << tainttracking::TaintTypeToString(key_taint_type) << " ValueTaintType " << tainttracking::TaintTypeToString(value_taint_type) << " MessageId " << ret << " KeyIs ";
          key->Print();
	        std::cout << " ValueIs ";
          value->Print();
          std::cout << std::endl; 

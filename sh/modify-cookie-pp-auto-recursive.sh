...
#usage: sudo bash modify-cookie-pp-auto-recursive.sh 0 100000 2 15
...
# rm -rf /tmp/*_com* /tmp/*_net* /tmp/*_org /tmp/*_io /tmp/*_edu* /tmp/*_uk*
cd ${SAVE_PATH}/src
if ((if_flush == 1))
then
...
do
    ...
    then
            ...
            out/PP/chrome ${url}/ --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --no-crankshaft --no-turbo --no-ignition" \
                     ...
                     --load-extension=/media/data1/zfk/Documents/crawler-extension-pp,/home/zfk/Documents/process-cookies/taintchrome/cookie_storage_log_extension,/home/zfk/Documents/process-cookies/taintchrome/cookie_storage_modify_extension,/home/zfk/Documents/process-cookies/taintchrome/pp_check_extension \
                     --new-window --no-sandbox --disable-gpu --disable-hang-monitor &>${SAVE_PATH}/src/${TAG}logs/${NAME}_log_file & #& pkill chrome > /dev/null &  #&>logs/${NAME}_log_file &
            #--user-data-dir=/tmp/${NAME}
...
done < <(grep . ${SAVE_PATH}/src/websites_to_modify_cookie.txt) #websites_to_pp_cookie_storage_0to1m.txt) #${SAVE_PATH}/src/websites_to_pp_cookie_storage_0to200k.txt

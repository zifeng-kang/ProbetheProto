...
#usage: sudo bash recursive-pp-key1key2-auto.sh 0 100000 2 22
...
then
        ...
        TAG="recursive_pp_key1key2_600kto1m_"
        if ((if_flush == 0))
        then
...
done < <(grep . ${SAVE_PATH}/src/websites_total_to_pp_pattern1_600kto1m.txt) #websites_total_to_pp_pattern1_0to600kplus.txt)  #websites_to_pp_pattern1_0to200k.txt #websites_total_to_pp_pattern1_600kto1m.txt

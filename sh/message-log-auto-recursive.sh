...
#usage: sudo bash message-log-auto-recursive.sh 0 100000 2 15
...
then
        ...
        # TAG="message_log_600kto1m_" #websites_to_pp_message_0to600kplus.txt
        if ((if_flush == 0))
        then
...
do
    ...
    then
            ...
            out/PP/chrome ${url}/ --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --no-crankshaft --no-turbo --no-ignition" \
                     --user-data-dir=/tmp/${NAME} --load-extension=/media/data1/zfk/Documents/crawler-extension-pp,/home/zfk/Documents/process-cookies/taintchrome/message_log_extension --new-window --no-sandbox --disable-gpu --disable-hang-monitor &>${SAVE_PATH}/src/${TAG}logs/${NAME}_log_file & #& pkill chrome > /dev/null &  #&>logs/${NAME}_log_file &
...
done < <(grep . ${SAVE_PATH}/src/websites_to_pp_message_0to600kplus.txt) #websites_to_pp_message_600kto1m_result_from_key1key2.txt)  # websites_to_pp_message_0to200k.txt

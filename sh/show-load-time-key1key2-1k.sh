...
then
        ...
        TAG="show_load_time_ppchrome_key1key2_1k_"
        if ((if_flush == 0))
        then
...
do
    ...
    then
            ...
            out/PP/chrome ${url}/?KEY1[KEY2]=VALUE\&KEY1.KEY2=VALUE\&KEY1[KEY2][KEY3]=VALUE --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --no-crankshaft --no-turbo --no-ignition" \
                     --user-data-dir=/tmp/${NAME} --load-extension=/home/zfk/Documents/process-cookies/taintchrome/show_load_time_extension --new-window --no-sandbox --disable-gpu --disable-hang-monitor &>${SAVE_PATH}/src/${TAG}logs/${NAME}_log_file & #& pkill chrome > /dev/null &  #&>logs/${NAME}_log_file &
...
done < <(grep . ${SAVE_PATH}/src/recrawl-ppchrome-1k.txt) #tranco_3Z3L_1k.csv)
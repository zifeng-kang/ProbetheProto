...
#usage: sudo bash check-pp-auto-recursive.sh 0 100000 2 22
...
then
        ...
        TAG="check_pp_pattern1_600kto1m_"
	if ((if_flush == 0))
        then
...
do
    ...
    then
            ...
            out/PP/chrome ${url} --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --no-crankshaft --no-turbo --no-ignition" \
                     --user-data-dir=/tmp/${NAME} --load-extension=/media/data1/zfk/Documents/crawler-extension-pp,/media/data1/zfk/Documents/sanchecker/src/pp_check_extension --new-window --no-sandbox --disable-gpu --disable-hang-monitor &>${SAVE_PATH}/src/${TAG}logs/${NAME}_log_file & #& pkill chrome > /dev/null &  #&>logs/${NAME}_log_file &
...
done < <(grep . ${SAVE_PATH}/src/websites_total_to_pp_pattern1_600kto1m.txt) #42_websites_url_src_to_pp_again_0to600kplus.txt)  #websites_total_to_pp_pattern1_0to600kplus.txt #websites_to_pp_pattern1_0to200k.txt

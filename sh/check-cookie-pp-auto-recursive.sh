...
#usage: sudo bash check-pp-auto-recursive.sh 0 1000 2 4
...
then
        ...
        TAG="check_cookie_pp_pattern1_0to200k_"
        if ((if_flush == 0))
        then
...
do
    ...
    then
            ...
            out/Bytecode/chrome ${url}/ --js-flags="--taint_log_file=${SAVE_PATH}/${TAG}crawl/$NAME --no-crankshaft --no-turbo --no-ignition" \
                     --user-data-dir=/tmp/${NAME} --load-extension=/media/data1/zfk/Documents/crawler-extension,/home/zfk/Documents/process-cookies/taintchrome/cookie_storage_log_extension --new-window --no-sandbox --disable-gpu --disable-hang-monitor &>${SAVE_PATH}/src/${TAG}logs/${NAME}_log_file & #& pkill chrome > /dev/null &  #&>logs/${NAME}_log_file &
...
done < ${SAVE_PATH}/src/websites_to_pp_cookie_storage_0to200k.txt
